...
2. Python 3.11 기반 가상환경(.venv) 자동 생성
3. 현재 쉘에 가상환경 진입 (source 방식)

⚠️ 주의: 반드시 source 또는 . 명령어로 실행해야 현재 쉘에 가상환경이 적용됩니다.

⚠️ Linux에서 command not found 에러가 발생할 경우 다음 명령어 우선 실행
