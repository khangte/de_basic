...
def crawl_to_csv(**context):
    ...
    # UTC timestamp (예: 20250619T123000) → datetime 변환
    execution_time_str = context['ts_nodash']
    utc_dt = datetime.strptime(execution_time_str, "%Y%m%dT%H%M%S")

    # → tz-aware UTC 시간으로 설정
    ...
    # → 파일명용 문자열로 포맷팅
    formatted_time = kst_dt.strftime("%Y%m%d_%H%M%S")
...
default_args = {
    # DAG 시작날짜
    # kst 로 변환할 경우, 이거도 수정해야됨. 함수에서 시간대만 바꾸면 에러남!
    'start_date' : datetime(2024, 1, 1, tzinfo=pytz.timezone("Asia/Seoul"))
}

# Airflow DAG 정의
with DAG(
    dag_id='crawl_quotes_to_csv2', # DAG 고유 ID
    default_args=default_args,
    schedule_interval='* * * * *', # 1분마다 실행(cron 표현식) https://crontab.guru/
    catchup=False, # 과거 진행한 실행분은 무시
    max_active_runs=1, # 동시에 하나의 실행만 허용
    tags=[""]
 ) as dag:
    # PythonOperator 를 사용하여 크롤링 함수 태스크 정의
    crawl_task = PythonOperator(
        task_id='crawl_to_csv2', # 태스크 ID
        python_callable=crawl_to_csv, # 실행할 함수
        provide_context=True, # Airflow 컨텍스트 함수
        execution_timeout=timedelta(minutes=10) # 태스크 최대 실행 시간
    )
